...
def get_main_menu_keyboard():
    ...
    builder.button(
        text="1. 🔍 Зарегистрировать покупку", callback_data="register_receipt"
    )
    ...
    builder.button(
        text="4. 🏆 Розыгрыш сертификата на 5000 руб.", callback_data="weekly_lottery"
    )
...
async def cmd_start(message: Message, session: AsyncSession):
    ...
    message_text = (
        f"Добро пожаловать в бот акции «ЛЕТО КРАСОТЫ. С заботой о чувствительной коже!»\n\n"
        f"Получите бережный уход за кожей по специальной цене с промо-кодом {code} и участвуйте в еженедельном розыгрыше сертификата OZON на 5 000 ₽!\n"
        "Для получения скидки при покупке товаров «Айсида» используйте промо-код "
        f"{code} при заказе товара:\n"
        "- на сайте «Планета Здоровья» https://planetazdorovo.ru/search/?q=Айсида\n"
        "- или получите скидку прямо в аптеке «Планета Здоровья».\n\n"
        "Ваша скидка при использовании промокода:\n"
        f"– {setting.discount_single} ₽ при покупке 1 средства «Айсида»\n"
        f"– {setting.discount_multi} ₽ при покупке 2 и более средств «Айсида»\n\n"
        "Скидка по промокоду применяется сразу в момент покупки.\n\n"
        "Как участвовать в еженедельном розыгрыше сертификата OZON на 5 000 руб.:\n"
        "1. После выкупа товара нажмите в меню «Зарегистрировать покупку».\n"
        "2. Сфотографируйте чек или введите данные чека вручную.\n"
        "3. Вы автоматически попадёте в розыгрыш сертификата OZON на 5 000 ₽, который проходит каждый понедельник.\n\n"
        "Готовы начать?\n"
        "Нажмите «Зарегистрировать покупку» или выберите «Меню», чтобы узнать больше."
    )
...
async def callback_about_aisida(callback: CallbackQuery):
    ...
    about_text = (
        ...
        "2. Крем для сухой и чувствительной кожи лица и тела «Айсида» (50 мл) или (250 мл)\n"
        ...
        "3. Шампунь «Айсида» с АСД для всех типов волос (250 мл)\n"
        ...
        "4. Бальзам-маска для сухой кожи головы «Айсида» (250 мл)\n"
        ...
        "5. Крем для ног «Айсида» с АСД и мочевиной 30 % (50 мл)\n"
