...
@app.get("/admin/lotteries/{lottery_id}/receipts")
async def get_lottery_receipts(
    lottery_id: int,
    week_start: str,
    week_end: str,
    current_admin: AdminUser = Depends(get_current_admin),
    ...
    """
    API для получения списка подходящих чеков для розыгрыша
    """
    ...
    try:
        ...
        logger.error(f"Ошибка при получении чеков для розыгрыша {lottery_id}: {str(e)}")
        return {"receipts": [], "error": str(e)}


@app.post("/admin/lotteries/{lottery_id}/set_winner")
async def set_manual_winner(
    lottery_id: int,
    receipt_id: int = Form(...),
    current_admin: AdminUser = Depends(get_current_admin),
    session: AsyncSession = Depends(get_db),
    ...
    """
    Установка победителя розыгрыша вручную
    """
    from models.weekly_lottery_model import WeeklyLottery
    from models.receipt_model import Receipt
    from logger import logger
    ...
    try:
        ...
        # Проверяем, что чек подходит для данной недели
        ...
        logger.info(
            f"Администратор {current_admin.username} установил победителя розыгрыша {lottery_id}: чек {receipt_id}, пользователь {receipt.user_id}"
        )

    except Exception as e:
        logger.error(
            f"Ошибка при установке победителя розыгрыша {lottery_id}: {str(e)}"
        )
